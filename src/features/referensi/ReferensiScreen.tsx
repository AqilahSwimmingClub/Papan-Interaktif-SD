...
export function ReferensiScreen() {
  ...
  if (!tingkat || !mapelKode) return <main className="halaman-kurikulum referensi-screen"><p className="label-data">Multi-referensi</p><h1>Referensi Pembelajaran</h1><section className="keadaan-kosong keadaan-kosong--fitur"><h2>Pilih kelas dan mata pelajaran</h2><p>Referensi disaring sesuai konteks aktif.</p><Link className="tombol-guru tombol-guru--utama" to={RUTE.kelas}>Pilih Kelas</Link></section></main>;

  return <main className="halaman-kurikulum referensi-screen" data-testid="referensi-screen">
    <header className="kop-kurikulum"><div><p className="label-data">Referensi resmi, panduan, dan buku</p><h1>Referensi Pembelajaran</h1><p>Satu CP/TP dapat memakai beberapa referensi. Hanya metadata dan ringkasan ruang lingkup yang disimpan.</p></div><Link className="tombol-guru tombol-guru--utama" to={RUTE.pemetaanReferensi}>Pemetaan Bab → TP</Link></header>
    ...
    <section className="referensi-grid">
      {daftar.map((item) => <article key={item.id} className={item.pilihan?.aktif ? 'dipilih' : ''}>
        ...
        <div className="referensi-card__aksi">{item.pilihan?.aktif ? <button type="button" onClick={() => void nonaktifkan(item)}>Lepaskan</button> : <button type="button" onClick={() => void ubah(item, false)}>Jadikan pendamping</button>}<button type="button" className="utama" onClick={() => void ubah(item, true)}>Pilih utama</button></div>
      </article>)}
    </section>
