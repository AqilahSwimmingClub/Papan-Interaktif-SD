...
export function ModeSiswaScreen() {
  ...
  async function cariSesi() {
    ...
    const [daftarSiswa, daftarKelompok] = await Promise.all([daftarSiswaKelas(ditemukan.kelas_id), daftarKelompokKelas(ditemukan.kelas_id)]);
  ...
  function batalTahan() { if (tahanRef.current) window.clearTimeout(tahanRef.current); tahanRef.current = null; }

  if (aktif && sesi) {
    const grup = kelompok.find((item) => item.id === aktif.kelompok_id);
    return <main className="mode-siswa mode-siswa--aktif" data-testid="mode-siswa"><header><span className="avatar-siswa">{aktif.nama.split(/\s+/).map((x) => x[0]).slice(0,2).join('')}</span><div><h1>{aktif.nama.split(' ')[0]}</h1><p>{grup ? `Kelompok ${grup.nama}` : 'Belum masuk kelompok'}</p></div><button type="button" onPointerDown={mulaiTahan} onPointerUp={batalTahan} onPointerLeave={batalTahan}>Tahan 3 detik untuk keluar</button></header><section className="siswa-sekarang"><span>Sekarang</span><h2>Sesi pembelajaran aktif</h2><p>{sesi.tp_id}</p><button type="button">Mulai</button></section><section className="ringkasan-siswa"><article><span>Tujuan tuntas</span><strong>—</strong><small>Menunggu hasil pertamamu</small></article><article><span>Lencanaku</span><strong>0</strong><small>Belum ada badge</small></article><article><span>Kelompokku</span><strong>{grup?.nama ?? '—'}</strong><small>{grup ? `${grup.anggota.length} anggota` : 'Guru akan mengatur kelompok'}</small></article></section><section className="latihan-siswa"><h2>Latihan untukku</h2><div className="kosong-siswa">Belum ada latihan dibagikan guru untuk sesi ini.</div></section></main>;
  }
