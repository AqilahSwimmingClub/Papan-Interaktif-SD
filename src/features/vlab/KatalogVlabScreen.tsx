  ...
  return (
    <main className="halaman-vlab" data-testid="katalog-vlab">
      <header className="vlab-katalog__kop">
        <div>
          <p className="label-data">IPAS Kelas V · master/percontohan</p>
          <h1>VLAB / Simulasi</h1>
          <p>Bab 1 dan Bab 2 masing-masing memiliki lima aktivitas VLAB yang dapat dijalankan langsung.</p>
        </div>
        <div className="vlab-katalog__angka"><div><strong>10</strong><span>VLAB Kelas V</span></div><div><strong>2</strong><span>bab aktif</span></div></div>
      </header>

      <section className="vlab5-catalog" aria-label="VLAB IPAS Kelas V Bab 1">
        <div className="vlab5-catalog__head"><div><span>IPAS Kelas V</span><h2>Bab 1 · Melihat karena Cahaya, Mendengar karena Bunyi</h2><p>Eksperimen cahaya dan bunyi dengan kontrol dan hasil pengamatan yang berbeda.</p></div><strong>5 playable</strong></div>
        <div className="vlab-grid">
          {bab1.map((profil)=>(
            <Link className="vlab-kartu vlab-kartu--kelas5" key={profil.kode} to={ruteVlab(profil.kode)} style={{ ['--vlab-warna' as string]: profil.warna }}>
              ...
              <dl><dt>Alat</dt><dd>{profil.alat.join(', ')}</dd><dt>Yang diamati</dt><dd>{profil.keluaran.join(', ')}</dd></dl><span className="vlab-kartu__buka">Mulai VLAB →</span>
            </Link>
          ))}
        </div>
      </section>
        ...
        <div className="vlab-grid">
          {VLAB_KELAS5_IPAS_BAB2.map((profil)=>(
            <Link className="vlab-kartu vlab-kartu--kelas5" key={profil.kode} to={`${RUTE.vlab}/kelas5-bab2/${profil.kode}`}>
              <span className="vlab-kartu__ikon" aria-hidden="true">{profil.ikon}</span><span className="vlab-rumpun">{profil.topik}</span><h2>{profil.nama}</h2><p>{profil.tujuan}</p><span className="vlab-kartu__buka">Mulai VLAB →</span>
            </Link>
          ))}
        </div>
      </section>
