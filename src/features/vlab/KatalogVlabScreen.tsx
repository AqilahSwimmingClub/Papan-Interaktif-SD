...
const BAGIAN = [
  ...
  {bab:6,judul:'Indonesiaku Kaya Raya',ringkas:'Kepulauan, keanekaragaman hayati, habitat, stok sumber daya alam, dan sumber terbarukan.',labs:VLAB_KELAS5_IPAS_BAB6},
...
export function KatalogVlabScreen() {
  ...
  return <main className="halaman-vlab" data-testid="katalog-vlab">
    ...
    <section className="vlab5-catalog"><div className="vlab5-catalog__head"><div><span>IPAS Kelas V</span><h2>Bab 1 · Melihat karena Cahaya, Mendengar karena Bunyi</h2><p>Eksperimen cahaya dan bunyi dengan kontrol dan hasil pengamatan yang berbeda.</p></div><strong>5 playable</strong></div><div className="vlab-grid">{bab1.map((profil)=><Link className="vlab-kartu vlab-kartu--kelas5" key={profil.kode} to={ruteVlab(profil.kode)} style={{ ['--vlab-warna' as string]: profil.warna }}><span className="vlab-kartu__ikon">{profil.ikon}</span><span className="vlab-rumpun">{LABEL_RUMPUN[profil.rumpun]}</span><strong className="vlab-kartu__judul">{profil.nama}</strong><p>Eksperimen langsung · ubah kontrol dan amati hasilnya.</p><span className="vlab-kartu__buka">Mulai VLAB →</span></Link>)}</div></section>
    {BAGIAN.map(b=><section className="vlab5-catalog" key={b.bab}><div className="vlab5-catalog__head"><div><span>IPAS Kelas V</span><h2>Bab {b.bab} · {b.judul}</h2><p>{b.ringkas}</p></div><strong>5 playable</strong></div><div className="vlab-grid">{b.labs.map((profil)=><Link className="vlab-kartu vlab-kartu--kelas5" key={profil.kode} to={`${RUTE.vlab}/kelas5-bab${b.bab}/${profil.kode}`}><span className="vlab-kartu__ikon">{profil.ikon}</span><span className="vlab-rumpun">{profil.topik}</span><strong className="vlab-kartu__judul">{profil.nama}</strong><p>{profil.tujuan}</p><span className="vlab-kartu__buka">Mulai VLAB →</span></Link>)}</div></section>)}
    <section aria-label="Semua laboratorium umum"><h2 className="vlab-subjudul">Semua laboratorium umum</h2><nav className="vlab-saring"><button type="button" aria-pressed={penyaring === 'semua'} onClick={()=>setPenyaring('semua')}>Semua lab</button>{rumpunTersedia.map(r=><button key={r} type="button" aria-pressed={penyaring===r} onClick={()=>setPenyaring(r)}>{LABEL_RUMPUN[r]}</button>)}</nav><div className="vlab-grid">{tersaring.map((profil)=><Link className="vlab-kartu" key={profil.kode} to={ruteVlab(profil.kode)} style={{ ['--vlab-warna' as string]: profil.warna }}><span className="vlab-kartu__ikon">{profil.ikon}</span><span className="vlab-rumpun">{LABEL_RUMPUN[profil.rumpun]}</span><h2>{profil.nama}</h2><p>{profil.tujuan}</p><span className="vlab-kartu__buka">Buka laboratorium →</span></Link>)}</div></section>
  </main>;
}
