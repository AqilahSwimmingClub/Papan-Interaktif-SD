...
export function KatalogVlabScreen() {
  ...
  return <main className="halaman-vlab" data-testid="katalog-vlab">
    <header className="vlab-katalog__kop"><div><p className="label-data">IPAS Kelas V · master/percontohan</p><h1>VLAB / Simulasi</h1><p>Bab 1–5 masing-masing memiliki lima aktivitas VLAB yang dapat dijalankan langsung.</p></div><div className="vlab-katalog__angka"><div><strong>25</strong><span>VLAB Kelas V</span></div><div><strong>5</strong><span>bab aktif</span></div></div></header>
    ...
    <section className="vlab5-catalog"><div className="vlab5-catalog__head"><div><span>IPAS Kelas V</span><h2>Bab 5 · Bagaimana Kita Hidup dan Bertumbuh</h2><p>Lima simulasi berbeda untuk pernapasan, volume paru, zat gizi, pencernaan, dan kebiasaan sehat.</p></div><strong>5 playable</strong></div><div className="vlab-grid">{VLAB_KELAS5_IPAS_BAB5.map((profil)=><Link className="vlab-kartu vlab-kartu--kelas5" key={profil.kode} to={`${RUTE.vlab}/kelas5-bab5/${profil.kode}`}><span className="vlab-kartu__ikon">{profil.ikon}</span><span className="vlab-rumpun">{profil.topik}</span><h2>{profil.nama}</h2><p>{profil.tujuan}</p><span className="vlab-kartu__buka">Mulai VLAB →</span></Link>)}</div></section>
