export const KOLOM_TEMPLATE_GURU = ['Nama', 'Username', 'Kelas', 'Rombel', 'Status'] as const;
export interface BarisGuruImpor { baris: number; nama: string; username: string; kelas: number[]; rombel: string; aktif: boolean; peran: 'guru'; valid: boolean; masalah: string[] }
...
export async function bacaExcelGuru(data: ArrayBuffer): Promise<BarisGuruImpor[]> {
  ...
  return XLSX.utils.sheet_to_json<Record<string, unknown>>(lembar, { defval: '', raw: false }).map((item, indeks) => {
    ...
    if (!kelas.length) masalah.push('Pilih sedikitnya satu kelas.');
    const rombel = kolom(item, 'Rombel').toLocaleUpperCase('id') || 'A';
    const aktif = kolom(item, 'Status').toLocaleLowerCase('id') === 'aktif';
    return { baris: indeks + 2, nama, username: akun, kelas, rombel, aktif, peran: 'guru', valid: masalah.length === 0, masalah };
  });
}

export async function unduhTemplateGuru(): Promise<void> {
  const XLSX = await import('xlsx');
  const lembar = XLSX.utils.aoa_to_sheet([[...KOLOM_TEMPLATE_GURU], ['Contoh Guru', 'guru.contoh', '1,2', 'A', 'Nonaktif']]);
